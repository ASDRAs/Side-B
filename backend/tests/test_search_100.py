...
_MOOD_DET = [
    ...
    pytest.param(
        "비 오는 날 듣기 좋은 노래", {"rainy-day", "chill", "acoustic"}, id="tc37"
    ),
    pytest.param(
        "코딩할 때 집중되는 음악", {"lo-fi", "instrumental", "focus"}, id="tc38"
    ),
    pytest.param(
        "공부할 때 틀어놓을 노래", {"focus", "instrumental", "ambient"}, id="tc39"
    ),
    pytest.param("운동할 때 신나는 음악", {"workout", "energetic", "dance"}, id="tc40"),
    pytest.param("퇴근길에 듣기 좋은 노래", {"commute", "chill", "pop"}, id="tc41"),
    pytest.param(
        "드라이브할 때 듣는 노래", {"road-trip", "indie", "feel-good"}, id="tc42"
    ),
    pytest.param("카페에서 나올 법한 음악", {"cafe", "acoustic", "chill"}, id="tc43"),
    pytest.param(
        "music for programming", {"lo-fi", "instrumental", "focus"}, id="tc55"
    ),
    pytest.param(
        "upbeat songs for running", {"workout", "energetic", "dance"}, id="tc56"
    ),
]
...
_LLM_DIRECT = [
    ...
    pytest.param(
        "임영웅 사랑은 늘 도망가", "Love Always Run Away Lim Young Woong", id="tc14"
    ),
    ...
    pytest.param(
        "Michael Jackson Billie Jean", "Billie Jean Michael Jackson", id="tc18"
    ),
    ...
    pytest.param(
        "Nirvana Smells Like Teen Spirit", "Smells Like Teen Spirit Nirvana", id="tc23"
    ),
    ...
    pytest.param(
        "지브리 인생의 회전목마", "Merry-Go-Round of Life Joe Hisaishi", id="tc31"
    ),
...
async def test_llm_direct(raw, expected_query):
    pq = ParsedQuery(
        type="direct", query=expected_query, tags=[], lastfm_candidates=[], raw=raw
    )
...
_LLM_MOOD = [
    pytest.param(
        "잠들기 전에 듣는 잔잔한 음악", ["sleep", "calm", "ambient"], id="tc44"
    ),
    ...
    pytest.param(
        "lo-fi hip hop for studying", ["lo-fi", "hip-hop", "study"], id="tc54"
    ),
    ...
    pytest.param(
        "dreamy bedroom pop", ["dream-pop", "bedroom-pop", "indie"], id="tc59"
    ),
    pytest.param(
        "energetic edm festival vibe", ["edm", "festival", "energetic"], id="tc60"
    ),
    ...
    pytest.param(
        "게임할 때 텐션 올리는 노래", ["gaming", "energetic", "electronic"], id="tc73"
    ),
    pytest.param(
        "집중력 올리는 백색소음 말고 음악",
...
async def test_llm_mood(raw, expected_tags):
    pq = ParsedQuery(
        type="mood", query=None, tags=list(expected_tags), lastfm_candidates=[], raw=raw
    )
    result = await parse_query(raw, _llm_ok(pq))
    assert result.type == "mood"
...
async def test_s1_artist_prefix_selects_correct_artist():
    ...
    idx = await score_and_select(
        "아이유 노래", pq, candidates, AsyncMock(), use_llm=False
    )
...
async def test_s2_lastfm_lookup_picks_original_over_karaoke():
    ...
    idx = await score_and_select(
        "Bohemian Rhapsody Queen", pq, candidates, AsyncMock(), use_llm=False
    )
...
async def test_s3_tag_overlap_beats_popularity_for_mood():
    ...
    idx = await score_and_select(
        "새벽감성 음악", pq, candidates, AsyncMock(), use_llm=False
    )
    assert idx == 1
...
async def test_s4_workout_tags_beat_sad_ballad_popularity():
    ...
    idx = await score_and_select(
        "운동할 때 신나는 음악", pq, candidates, AsyncMock(), use_llm=False
    )
    assert idx == 1
...
async def test_s5_popularity_is_tiebreaker_when_tag_overlap_equal():
    ...
    idx = await score_and_select(
        "너무 유명하지 않은 잔잔한 노래", pq, candidates, AsyncMock(), use_llm=False
    )
    assert idx == 0
