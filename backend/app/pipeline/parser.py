...
async def parse_query(
    raw: str, llm: GeminiClient, use_deterministic: bool = True
) -> ParsedQuery:
...
async def _parse_query_uncached(
    raw: str, llm: GeminiClient, use_deterministic: bool = True
) -> ParsedQuery:
    ...
    try:
        parsed = await asyncio.wait_for(
            llm.parse_query(raw), timeout=LLM_PARSE_TIMEOUT_SECONDS
        )
        ...
        if use_deterministic:
            ...
            if fallback:
                return fallback
        return ParsedQuery(
            type="direct", query=raw, tags=[], lastfm_candidates=[], raw=raw
        )

    if (
        use_deterministic
        and parsed.type == "direct"
        and _looks_like_activity_query(raw)
    ):
        mood = _activity_mood_query(raw)
        if mood:
            return mood
...
def _activity_mood_query(raw: str) -> ParsedQuery | None:
    ...
    if re.search(
        r"\b(music|songs?)\s+for\s+(programming|coding|work|concentration|focus)\b",
        lowered,
    ):
        return ParsedQuery(
            type="mood",
    ...
    if re.search(
        r"\b((music|songs?)\s+for\s+)?(workout|working out|running|gym)(\s+(music|playlist|songs?))?\b",
        lowered,
    ):
        return ParsedQuery(
            type="mood",
    ...
    if re.search(r"\b(music|songs?)\s+for\s+(driving|road trip|commute)\b", lowered):
        return ParsedQuery(
            type="mood",
            query=None,
            tags=["road-trip", "indie", "feel-good"],
            raw=raw,
            confidence=1.0,
        )
    ...
    for keywords, tags in ACTIVITY_MOOD_RULES:
        if any(word in raw for word in keywords):
            return ParsedQuery(
                type="mood", query=None, tags=tags, raw=raw, confidence=1.0
            )
...
def _artist_only_query(raw: str) -> ParsedQuery | None:
    ...
    tags = (
        ["k-pop", "korean"]
        if artist in {"IU", "BTS", "NewJeans", "BLACKPINK", "fromis_9"}
        else []
    )
    return ParsedQuery(
        type="direct",
...
def _known_direct_query(raw: str) -> ParsedQuery | None:
    ...
    if ("아이유" in raw or "iu" in lowered) and (
        "너랑나" in compact or "너랑 나" in raw
    ):
        return ParsedQuery(
            type="direct",
            query="너랑나 IU",
